...
function formatSecurityFeature(feature: SecurityFeature): {
  label: string;
  color: string;
} {
  switch (feature.type) {
    case "checksum":
      return {
        label: feature.algorithm
          ? `checksum (${feature.algorithm})`
          : "checksum",
...
export function InfoPane({ tool, toolMeta }: InfoPaneProps) {
  ...
  const hasMetadata =
    ...
    (toolMeta.license ||
      toolMeta.homepage ||
      toolMeta.repo_url ||
      toolMeta.authors?.length ||
      toolMeta.security?.length);
  const hasLinks =
    toolMeta &&
    (toolMeta.package_urls || toolMeta.aqua_link || toolMeta.backends?.length);
  const hasGithub =
    ghData && (ghData.stars > 0 || (ghData.topics && ghData.topics.length > 0));
  const showGithubPlaceholder =
    !ghData && !ghLoading && !authenticated && parsed !== null;
    ...
    <div class="space-y-4">
      ...
      {(hasMetadata || hasLinks || hasGithub || showGithubPlaceholder) && (
        <div class="bg-dark-800 border border-dark-600 rounded-lg p-4 space-y-4">
              ...
              {toolMeta.homepage &&
                (() => {
                  try {
                    ...
                    return (
                      <div class="flex">
                        <dt class="text-gray-500 w-24 flex-shrink-0">
                          Homepage
                        </dt>
                        <dd>
                          <a
              ...
              {toolMeta.authors && toolMeta.authors.length > 0 && (
                <div class="flex">
                  ...
                  <dd class="text-gray-300">
                    ...
                    {toolMeta.authors.length > 3 &&
                      ` +${toolMeta.authors.length - 3} more`}
                  </dd>
                </div>
              )}
          ...
          {(hasMetadata || hasLinks) &&
            (hasGithub || showGithubPlaceholder) && (
              <div class="border-t border-dark-600" />
            )}
          ...
          {showGithubPlaceholder && (
            <div class="space-y-3">
              ...
              <div class="flex flex-wrap gap-1 opacity-40 select-none">
                <span class="px-2 py-0.5 bg-dark-700 rounded text-xs text-gray-600">
                  topic
                </span>
                <span class="px-2 py-0.5 bg-dark-700 rounded text-xs text-gray-600">
                  topic
              ...
              <div class="flex flex-wrap gap-x-4 gap-y-1 text-sm">
                ...
                {ghData.forks > 0 && (
                  <span class="text-gray-400">
                    ...
                    {ghData.forks.toLocaleString()} forks
                  </span>
                )}
                {ghData.open_issues > 0 && (
                  <span class="text-gray-400">
                    <span class="text-green-400">○</span>{" "}
                    {ghData.open_issues.toLocaleString()} issues
                  </span>
                )}
              </div>
              {/* Secondary info row */}
              <div class="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                {ghData.language && <span>{ghData.language}</span>}
                {ghData.pushed_at && (
